	...
	public void mouseClicked(MouseEvent e) {
		...
		if (str.equals("人人对战")) {
			...
			if (x >= X && y >= Y && x <= X + (Colum - 1) * SIZE && y <= Y + (Row - 1) * SIZE) {
				int c = (y - Y + SIZE / 2) / SIZE;
				int r = (x - X + SIZE / 2) / SIZE;
				if (arrays[r][c] == 0) {
					y = c * SIZE + X - Chess_SIZE / 2;
					x = r * SIZE + Y - Chess_SIZE / 2;
					if (flag) {
...
//				g.fillOval(x, y, Chess_SIZE, Chess_SIZE);
			...
			if (x >= X && y >= Y && x <= X + (Colum - 1) * SIZE && y <= Y + (Row - 1) * SIZE) {
				...
				int r = (x - Y + SIZE / 2) / SIZE;
				int c = (y - X + SIZE / 2) / SIZE;
				if (arrays[r][c] == 0) {
					x = r * SIZE + X - Chess_SIZE / 2;
					y = c * SIZE + Y - Chess_SIZE / 2;
